...
function Utility() {
  var def = {
    ...
    setColorScheme: function (c1) {
      ...
      var css =
        "" +
        ".odd  .tile-1 .inner { background-color: " +
        c1 +
        "; }" +
        ".even .tile-1 .inner { background-color: " +
        c1lum +
        "; }" +
        ".odd  .tile-2 .inner { background-color: " +
        c2 +
        "; }" +
        ".even .tile-2 .inner { background-color: " +
        c2lum +
        "; }";
      Utils.createCSS(css);
    },
...
var Colors = new (function () {
  ...
  function colorToRgb(color) {
    if (!isNaN(color)) color = PALETTE[color];
  ...
  function getComplementary(rgb) {
    var asHex = false;
    if (typeof rgb == "string") asHex = true;
    if (asHex) rgb = hexToRgb(rgb);
    var comp = rgbToHsv(rgb);
    comp.hue = hueShift(comp.hue, 180.0);
    var result = hsvToRgb(comp);
    if (asHex) result = rgbToHex(result);
    return result;
  }
