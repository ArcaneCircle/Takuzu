...
function Grid(size, height, id) {
  ...
  let width = size,
    ...
    quality = 0,
    tileToSolve = null;
  const maxPerRow = Math.ceil(width / 2),
  ...
  let state = (self.state = new State(this));
  let hint = (self.hint = new Hint(this));
  ...
  function domRenderer(direct) {
    ...
    if (Game.debug) console.log("rendering...");
    rendered = false;
    let html =
      '<table data-grid="' + id + '" id="grid" cellpadding=0 cellspacing=0>';
  ...
  let tile = getTile; // other reference
  ...
  function ease(percentage) {
    const emptyTiles = getEmptyTiles(),
      easeCount = percentage
        ? Math.floor((percentage / 100) * emptyTiles.length)
        : 1;
  ...
  function solve(isGenerating, stepByStep) {
    ...
    while (attempts++ < totalAtt) {
      ...
      if (
        tileToSolve &&
        tileChanged &&
        tileToSolve.x == tileChanged.x &&
        tileToSolve.y == tileChanged.y
      ) {
    ...
    for (var i = 0; i < width; i++) {
      ...
      if (info.isFull) {
        if (cols[info.str]) {
          ...
          if (hint.active) hint.doubleColFound.push(cols[info.str] - 1, i);
          return false;
        } else {
          info.unique = true;
          cols[info.str] = i + 1;
      ...
      if (info.isFull) {
        if (rows[info.str]) {
          ...
          if (hint.active) hint.doubleRowFound.push(rows[info.str] - 1, i);
          return false;
        } else {
          info.unique = true;
          rows[info.str] = i + 1;
  ...
  function markRow(y) {
    for (var x = 0; x < width; x++) tile(x, y).mark();
    return self;
  }
  ...
  function markCol(x) {
    for (var y = 0; y < height; y++) tile(x, y).mark();
    return self;
  }

  function unmarkCol(x) {
    for (var y = 0; y < height; y++) tile(x, y).unmark();
    return self;
  }
