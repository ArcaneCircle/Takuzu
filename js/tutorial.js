/*
 * Tutorial
 * The game tutorial with its messages and required tile(s) to tap.
 * (c) 2014 Q42
 * http://q42.com | @q42
...
var TutorialMessages = {
  ...
  7: {
    msg: "Colums have an equal number of each color too.",
    tiles: [[1, 3, 2]],
    cols: [1],
  },
  8: {
    msg: "You should be able to know what colors these are...",
    tiles: [
      [2, 3, 1],
      [0, 3, 1],
    ],
  ...
  9: {
    msg: "One more thing...<br>No two rows are the same.",
    tiles: [
      [0, 2, 2],
      [3, 2, 1],
    ],
...
window.Tutorial = new (function () {
  ...
  function tapTile(tile) {
    ...
    $(tilesToTapThisStep).each(function () {
      if (tile.x == this.x && tile.y == this.y) tapAllowed = true;
    });
  ...
  function checkStepCompleted() {
    ...
    $(TutorialMessages[step].tiles).each(function () {
      var x = this[0],
        ...
        value = this[2];
      if (tile.value != value) completed = false;
    });
    if (!completed) return;
