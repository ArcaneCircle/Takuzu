...
function Tile(value, grid, index) {
  ...
  function collect(hint) {
    ...
    for (var v = 1; v <= 2; v++) {
      ...
      if (
        (move(Directions.Left).value == v &&
          move(Directions.Right).value == v) ||
    ...
    if (rowInfo.nr1s >= grid.maxPerRow) {
      possibleValues = [2];
      if (hint && hint.active) hint.mark(self, HintType.RowMustBeBalanced);
      return self;
    }
    ...
    if (colInfo.nr1s >= grid.maxPerCol) {
      possibleValues = [2];
      if (hint && hint.active) hint.mark(self, HintType.ColMustBeBalanced);
      return self;
    }
